...
export function buildDRE(saidas: Transaction[], entradas: Transaction[]): DREResult {
  ...
  const lines: DRELine[] = [
    ...
    { linha: 4,  descricao: '(-) Custo de Mercadorias Vendidas (CMV)',   sinal: '-',  rowStyle: 'despesa',   total: totTxs(grpCOGS),           months: mL4,  groups: groupBySubcat(grpCOGS),      transactions: grpCOGS,      expandable: grpCOGS.length > 0 },
